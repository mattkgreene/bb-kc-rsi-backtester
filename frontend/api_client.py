...
def enqueue_optimize(payload: dict[str, Any]) -> dict[str, Any]:
    resp = request_json("POST", "/v1/optimize", payload=payload)
...
def enqueue_discover(payload: dict[str, Any]) -> dict[str, Any]:
    resp = request_json("POST", "/v1/discover", payload=payload)
    if not isinstance(resp, dict) or "job" not in resp:
...
def enqueue_leaderboard_refresh(payload: Optional[dict[str, Any]] = None) -> dict[str, Any]:
    resp = request_json("POST", "/v1/leaderboard/refresh", payload=(payload or {}))
    if not isinstance(resp, dict) or "job" not in resp:
...
def enqueue_patterns_refresh(payload: Optional[dict[str, Any]] = None) -> dict[str, Any]:
    resp = request_json("POST", "/v1/patterns/refresh", payload=(payload or {}))
    if not isinstance(resp, dict) or "job" not in resp:
...
def get_job(job_id: int) -> dict[str, Any]:
    resp = request_json("GET", f"/v1/jobs/{int(job_id)}")
...
def get_job_events(job_id: int, *, limit: int = 200) -> list[dict[str, Any]]:
    resp = request_json("GET", f"/v1/jobs/{int(job_id)}/events?limit={int(limit)}")
...
def cancel_job(job_id: int) -> dict[str, Any]:
    resp = request_json("POST", f"/v1/jobs/{int(job_id)}/cancel", payload={})
    if not isinstance(resp, dict) or "id" not in resp:
...
def get_discovery_stats() -> dict[str, Any]:
    resp = request_json("GET", "/v1/discovery/stats")
...
def get_patterns(*, min_confidence: float = 0.3) -> list[dict[str, Any]]:
    resp = request_json("GET", f"/v1/patterns?min_confidence={float(min_confidence)}")
...
def get_leaderboard_latest() -> dict[str, Any]:
    resp = request_json("GET", "/v1/leaderboard")
