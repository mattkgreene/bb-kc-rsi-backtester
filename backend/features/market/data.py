...
from .ohlcv_cache import get_cache_bounds, read_ohlcv_range, upsert_ohlcv, init_ohlcv_cache
...
def _project_root() -> Path:
    ...
    Assumes this file lives at backend/features/market/data.py.
    """
...
def _default_ohlcv_cache_dir() -> Path:
    ...
    Stored under <project>/data/ohlcv_cache so it persists across app restarts
...
def fetch_ohlcv_range_db_cached(
    ...
    *,
    ...
    bounds = get_cache_bounds(exchange, symbol, timeframe, db_path)
    ...
    for m_start, m_end in missing_ranges:
        ...
        if fetched is None or fetched.empty:
            continue
        upsert_ohlcv(exchange, symbol, timeframe, fetched, db_path)

    return read_ohlcv_range(exchange, symbol, timeframe, s, e, db_path)
