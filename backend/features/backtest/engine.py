"""
Backtesting engine for the BB+KC+RSI long/short mean-reversion strategy.
...
The strategy supports both short and long trades. Short entries trigger
on upper-band touches with high RSI, while long entries trigger on
lower-band touches with low RSI. Exits occur on mean reversion to the
mid band or the outer band, or via stop loss/trailing stop/time stop.
...
from ..market.indicators import add_bb_kc_rsi
from ..market.utils import cmp, vectorized_cmp
...
def run_strategy_loop(
    ...
    *,
    ...
    """
    Execute the long/short trading strategy on prepared dataset.
    ...
    Args:
        ...
        Entry Conditions:
            ...
            rsi_max: Maximum RSI value for long entries (oversold).
            rsi_ma_max: Maximum RSI MA value for long entries.
            ...
            trade_direction: 'Short', 'Long', or 'Both'.
        
        Exit Conditions:
            exit_channel: Channel for signal exit ('BB' or 'KC').
            exit_level: Level for signal exit ('mid' or outer band). For longs, 'lower' maps to upper.
    ...
    if rsi_max is None:
        rsi_max = 100.0 - float(rsi_min)
    if rsi_ma_max is None:
        rsi_ma_max = 100.0 - float(rsi_ma_min)

    rsi_max = _clamp_rsi(rsi_max)
    rsi_ma_max = _clamp_rsi(rsi_ma_max)
    ...
    # Use HIGH for short touches, LOW for long touches.
    touch_kc_short_vec = (high >= kc_up) & ~np.isnan(kc_up)
    touch_bb_short_vec = (high >= bb_up) & ~np.isnan(bb_up)
    touch_kc_long_vec = (low <= kc_low) & ~np.isnan(kc_low)
    touch_bb_long_vec = (low <= bb_low) & ~np.isnan(bb_low)

    if entry_band_mode == "KC":
        band_ok_short_vec = touch_kc_short_vec
        band_ok_long_vec = touch_kc_long_vec
    elif entry_band_mode == "BB":
        band_ok_short_vec = touch_bb_short_vec
        band_ok_long_vec = touch_bb_long_vec
    elif entry_band_mode == "Both":
        band_ok_short_vec = touch_kc_short_vec & touch_bb_short_vec
        band_ok_long_vec = touch_kc_long_vec & touch_bb_long_vec
    else:  # "Either"
        band_ok_short_vec = touch_kc_short_vec | touch_bb_short_vec
        band_ok_long_vec = touch_kc_long_vec | touch_bb_long_vec

    # RSI conditions (vectorized)
    rsi_short_ok_vec = ~np.isnan(rsi) & (rsi >= rsi_min)
    rma_short_ok_vec = ~np.isnan(rma) & (rma >= rsi_ma_min)
    rsi_long_ok_vec = ~np.isnan(rsi) & (rsi <= rsi_max)
    rma_long_ok_vec = ~np.isnan(rma) & (rma <= rsi_ma_max)

    # RSI relation (vectorized)
    if use_rsi_relation:
        rsi_relation_short_vec = vectorized_cmp(rsi, rma, rsi_relation) & ~np.isnan(rsi) & ~np.isnan(rma)
        rsi_relation_long_vec = vectorized_cmp(rsi, rma, _invert_relation(rsi_relation)) & ~np.isnan(rsi) & ~np.isnan(rma)
    else:
        rsi_relation_short_vec = np.ones(n, dtype=bool)
        rsi_relation_long_vec = np.ones(n, dtype=bool)

    # Combined entry signals: all conditions must be true
    entry_signal_short_vec = (
        band_ok_short_vec
        & rsi_short_ok_vec
        & rma_short_ok_vec
        & rsi_relation_short_vec
        & (close > 0)
    )
    entry_signal_long_vec = (
        band_ok_long_vec
        & rsi_long_ok_vec
        & rma_long_ok_vec
        & rsi_relation_long_vec
        & (close > 0)
    )

    # --- Helper Functions ---

    def compute_stop_price(entry_px: float, i: int, side: str) -> Optional[float]:
    ...
    def update_trailing_stop(pos_dict: dict, i: int) -> None:
    ...
    direction = str(trade_direction or "Short").strip().lower()
    if direction in {"long", "long-only", "long only"}:
        allow_long = True
        allow_short = False
    elif direction in {"short", "short-only", "short only"}:
        allow_long = False
        allow_short = True
    else:
        allow_long = True
    ...
    for i in range(n):
        ...
        if pos is None and equity > 0:
            ...
            if (
                ...
                short_signal = entry_signal_short_vec[i] if allow_short else False
                long_signal = entry_signal_long_vec[i] if allow_long else False
                side = None
                if short_signal and long_signal:
                    side = "Short" if allow_short else "Long"
                elif short_signal:
                    side = "Short"
                elif long_signal:
                    side = "Long"
                ...
                if side:
                    ...
                    if use_stop:
                        ...
                        stop_price = compute_stop_price(entry_px, i, side)
                    ...
                    if (
                        ...
                        if side == "Short":
                            ...
                            # For long: Equity(P) = EQ0 + (P - Entry) * size
                            # Liquidation when Equity(P) <= N * m
                            # Solving: P = Entry + (N*m - EQ0) / size
                            liq_price = entry_px + (N * m - EQ0) / size
        ...
        if pos is not None:
            ...
            # 4) Check signal-based exit (price reaches mid/outer band)
            if exit_reason is None:
                ...
                if exit_level == "mid":
                    ...
                    thresh = lower_arr[i] if side == "Short" else upper_arr[i]

                if not math.isnan(thresh):
                    if side == "Short" and close[i] <= thresh:
                        exit_reason = "signal_exit"
                        exit_px = float(close[i])
                    elif side == "Long" and close[i] >= thresh:
                        exit_reason = "signal_exit"
                        exit_px = float(close[i])
...
def finalize_trades(
    ...
    *,
    ...
    # Calculate per-unit return (price move percentage) based on side
    side = t["Side"].fillna("Short")
    entry_px = pd.to_numeric(t["EntryPrice"], errors="coerce")
    exit_px = pd.to_numeric(t["ExitPrice"], errors="coerce")
    long_mask = side == "Long"
    t["ReturnPct"] = np.where(long_mask, (exit_px - entry_px) / entry_px, (entry_px - exit_px) / entry_px)
...
def run_backtest(
    ...
    """
    Run a complete backtest with the BB+KC+RSI long/short strategy.
    ...
    Args:
        ...
        rsi_*: RSI parameters (min for shorts, max for longs).
        trade_direction: 'Short', 'Long', or 'Both'.
    ...
    trades_raw, equity_curve = run_strategy_loop(
        ...
        rsi_max=rsi_max, rsi_ma_max=rsi_ma_max,
